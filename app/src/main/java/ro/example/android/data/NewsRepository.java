...
public class NewsRepository {
    ...
    public Observable<List<ArticleEntity>> listenToAllArticles(String category) {
        return this.database.articleDao().loadArticles(category)
    ...
    //emits only if event completed or error appeared
    public Completable fetchArticlesByCategory(String category) {
        // First fetch the articles from the webservice
        return webService.fetchCategory(category)
                // ... then ...
                .flatMapCompletable((List<ArticleDto> articleDtos) -> {
                    ...
                    articleEntities.forEach(articleEntity -> articleEntity.setCategory(category));
    ...
    public Completable fetchHeadLines(){
        return webService.fetchTopHeadlines().flatMapCompletable(articleDtos ->{
            List<ArticleEntity> articleEntities = articleDtos.stream()
                    .map(articleDto -> articleDto.toEntity())
                    .collect(Collectors.toList());
            articleEntities.forEach(articleEntity -> articleEntity.setCategory("headlines"));
            return database.articleDao().insertAllArticles(articleEntities);
        } ).subscribeOn(Schedulers.io());
    }
